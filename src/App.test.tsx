...
describe("Phase 1 manual flow", () => {
  ...
  it("continues from selection to timeline and export", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "推しごとを、ひと目で。" })).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "デザイン" }));
    await user.click(screen.getByRole("button", { name: "オーシャン" }));
    await user.click(screen.getByRole("button", { name: "表示内容" }));
    await user.type(screen.getByLabelText("タイトル"), "My festival");
    await user.click(screen.getByRole("checkbox", { name: "日付" }));
    await user.click(screen.getByRole("button", { name: "サイズ" }));
    await user.click(screen.getByRole("button", { name: /SNS横長/ }));
    const width = screen.getByRole("spinbutton", { name: "幅 (px)" });
    await user.clear(width);
    await user.type(width, "1200");
    await user.tab();
    expect(width).toHaveValue(1200);
    const preview = screen.getByAltText("生成したタイムラインのプレビュー");
    const svg = decodeURIComponent(preview.getAttribute("src")!);
    expect(svg).toContain('width="1200" height="900"');
    expect(svg).toContain("My festival");
    expect(svg).toContain("#3567ba");
    await user.clear(width);
    await user.type(width, "100");
    await user.tab();
    expect(width).toHaveValue(320);
    await user.click(screen.getByRole("button", { name: "表示内容" }));
    expect(screen.getByLabelText("タイトル")).toHaveValue("My festival");
    expect(screen.getByRole("checkbox", { name: "日付" })).not.toBeChecked();
