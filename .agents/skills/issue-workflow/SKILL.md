...
3. `bot:execute`がなければ、ユーザーが実行を依頼していても停止する。人間によるラベル付与だけを実装開始の許可として扱う。
4. `needs-triage`付きIssueは停止する。`bot:execute`があってもtriageを省略しない。
5. 他の`bot:*`状態ラベルがなく、対象Issueに`bot:execute`だけがあることを再取得して確認する。実装やworktree作成の前に、他のラベルを維持したまま`bot:execute`を`bot:running`へ置き換える単一のラベル更新を行い、更新後の状態を再取得する。更新失敗、競合、または期待外の状態なら停止し、`bot:running`を確認できるまで続行しない。
6. `AGENTS.md`と`docs/SPEC.md`の関連箇所、既存実装、テストを読み、Issueとの一致を確認する。矛盾は暗黙に解消せず報告して停止する。ただしIssueが具体的なSPEC更新を完了条件として明示している場合、その更新を最初の実装作業に含める。
